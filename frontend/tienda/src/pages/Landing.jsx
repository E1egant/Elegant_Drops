...
export default function Landing() {
    ...
    useEffect(() => {
        axios.get('/api/tienda/resenas').then(res => setResenas(res.data)).catch(() => {})
    }, [])
    ...
    const redes = [
        { label: 'Instagram', url: 'https://www.instagram.com/elegant_drop/', icon: 'fa-brands fa-instagram', color: '#E1306C' },
        { label: 'TikTok', url: 'https://www.tiktok.com/@elegant_drops', icon: 'fa-brands fa-tiktok', color: '#ffffff' },
        { label: 'Facebook', url: 'https://www.facebook.com/profile.php?id=61560668613953', icon: 'fa-brands fa-facebook', color: '#1877F2' },
        { label: 'Correo', url: 'https://mail.google.com/mail/?view=cm&to=ventas.elegantdrops@gmail.com&su=Consulta%20Elegant%20Drops', icon: 'fa-solid fa-envelope', color: 'var(--gold)' },
    ]
    ...
    return (
        <div style={{ minHeight: '100vh', background: 'var(--bg-main)' }}>
            ...
            <nav style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    ...
                    <div style={{ borderLeft: '1px solid var(--border-soft)', paddingLeft: 16 }}>
                        ...
                        <p style={{ fontSize: 7, letterSpacing: '0.3em', color: 'var(--gold)', fontWeight: 800, textTransform: 'uppercase', marginTop: 3 }}>Decants & Fragrances</p>
                    </div>
                </div>
                <button onClick={() => setCarritoAbierto(true)} style={{
                    background: 'transparent', border: 'none', color: 'var(--text-muted)',
                    cursor: 'pointer', position: 'relative', padding: 8, transition: 'color 0.2s'
                }}
                    ...
                    {count > 0 && (
                        <span style={{
                            position: 'absolute', top: 2, right: 2, background: 'var(--gold)', color: '#0e0d0b',
                            fontSize: 9, fontWeight: 900, width: 16, height: 16, borderRadius: '50%',
            ...
            <div style={{ padding: '48px 24px 32px', textAlign: 'center' }}>
                ...
                <div style={{ maxWidth: 480, margin: '0 auto', position: 'relative' }}>
                    <div style={{ position: 'relative' }}>
                        <i className="fa-solid fa-magnifying-glass" style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-faint)', fontSize: 14 }}></i>
                        <input className="input" style={{ paddingLeft: 44, borderRadius: 100 }}
                               placeholder="Buscar fragancia, marca..." value={busqueda} onChange={e => setBusqueda(e.target.value)} />
                    </div>
                    {(resultados.length > 0 || buscando) && (
                        <div style={{
                            position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 8,
                            background: 'var(--bg-card)', border: '1px solid var(--border-medium)',
                            borderRadius: 16, overflow: 'hidden', zIndex: 100, boxShadow: '0 20px 40px rgba(0,0,0,0.5)'
                        }}>
                            {buscando ? (
                                <p style={{ padding: '16px', fontSize: 11, color: 'var(--text-faint)', textAlign: 'center' }}>Buscando...</p>
                            ) : resultados.map(f => (
                                <div key={f.id}
                                     onClick={() => { setBusqueda(''); setResultados([]); navigate(f.categoria === 'COMPLETO' ? '/completos' : '/decants') }}
                                     style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', cursor: 'pointer', borderBottom: '1px solid var(--border-soft)', transition: 'background 0.2s' }}
            ...
            <div style={{ maxWidth: 600, margin: '0 auto', padding: '0 16px 48px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                {secciones.map(s => (
                    <div key={s.key} onClick={() => navigate(s.ruta)}
                         style={{ position: 'relative', borderRadius: 20, overflow: 'hidden', height: 220, cursor: 'pointer', border: '1px solid var(--border-soft)', transition: 'all 0.3s' }}
                         onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(201,150,122,0.5)'; e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 20px 40px rgba(0,0,0,0.4)' }}
                        ...
                        <div style={{ position: 'absolute', inset: 0, background: 'var(--bg-card)' }}>
                            <img src={s.imagen} alt={s.label} onError={e => e.target.style.display = 'none'} style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.5 }} />
                        ...
                        <div style={{ position: 'relative', zIndex: 1, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 32px' }}>
                            <h2 className="font-luxury" style={{ fontSize: 'clamp(18px, 4vw, 26px)', textTransform: 'uppercase', color: 'var(--text-pure)', marginBottom: 8 }}>{s.label}</h2>
                            <p style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: 20 }}>{s.subtexto}</p>
                            <span style={{ display: 'inline-block', border: '1px solid var(--gold)', color: 'var(--gold)', fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.15em', padding: '8px 20px', borderRadius: 100, width: 'fit-content' }}>
                Ver {s.label} →
              </span>
                        </div>
                    </div>
            ...
            {resenas.length > 0 && (
                <div style={{ padding: '48px 24px', maxWidth: 700, margin: '0 auto' }}>
                    <p style={{ fontSize: 9, letterSpacing: '0.3em', color: 'var(--gold)', textTransform: 'uppercase', marginBottom: 8, textAlign: 'center', fontWeight: 800 }}>
                        Lo que dicen nuestros clientes
                    </p>
                    <h2 className="font-luxury" style={{ fontSize: 28, textTransform: 'uppercase', textAlign: 'center', marginBottom: 32 }}>Reseñas</h2>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        {resenas.slice(0, 5).map(r => (
                            <div key={r.id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border-soft)', borderRadius: 16, padding: '20px 24px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                                    <div>
                                        <p style={{ fontWeight: 700, fontSize: 13 }}>{r.nombre}</p>
                                        <div style={{ display: 'flex', gap: 2, marginTop: 4 }}>
                                            {[1,2,3,4,5].map(n => (
                                                <span key={n} style={{ color: n <= r.calificacion ? 'var(--gold)' : 'var(--text-faint)', fontSize: 14 }}>★</span>
                                            ))}
                                        </div>
                                    </div>
                                    <p style={{ fontSize: 10, color: 'var(--text-faint)' }}>{new Date(r.fecha).toLocaleDateString('es-CL')}</p>
                                </div>
                                <p style={{ fontSize: 13, color: 'var(--text-muted)', lineHeight: 1.6, fontStyle: 'italic' }}>"{r.comentario}"</p>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 28, marginBottom: 24 }}>
                    {redes.map((red, i) => (
                        <a key={i} href={red.url} target="_blank" rel="noreferrer"
                           style={{ color: 'var(--text-faint)', textDecoration: 'none', fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', transition: 'color 0.2s', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}
